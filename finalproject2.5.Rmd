...
``` {r }
#Calculate forward returns and add the new variable Asset_1_HRet_10_forward to bw_full
...
X <- model.matrix(Asset_1_BRet_10_forward ~ ., data = bw_full)
X_train <- model.matrix(Asset_1_BRet_10_forward ~ ., data = data_ridge_train)
X_test <- model.matrix(Asset_1_BRet_10_forward ~ ., data = data_ridge_test)
...
cor(as.matrix(cbind(train_pred_ridge, data_ridge_train$Asset_1_BRet_10_forward)))
#Out-of-sample correlation
cor(as.matrix(cbind(test_pred_ridge, data_ridge_test$Asset_1_BRet_10_forward)))
```